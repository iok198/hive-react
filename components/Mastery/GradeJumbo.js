...
class GradeJumbo extends React.Component {
  ...
  _getMastery(mObj,page){
    ...
    var studentRows = Object.keys(rowsByStu).map((key1,id1) => (<MasteryStuTR key={key1} stuData={rowsByStu[key1]} colOffset={id1} >
      {Object.keys(rowsByStu[key1]).slice(page,page+5).map((key2,id2) => {
    ...
    var pagination = (<ul className="pagination">{LOs.map((LO,id3) => ((id3%5 == 0) ? (<li key={LO + '1'}><a href="#" onClick={console.log(mObj)}>{(id3/5) + 1}</a></li>) : ''))}</ul>)
